...
Enjeu is a solo boss rush played with 111 printed cards, one die, and whatever
...
- **Learn**: a fourteen-slide deck in two chapters: Basics is everything a first fight needs (one rule, six cards, a turn, the check, the boss, keeping count, what you need to own), then a Try it slide that hands you the Play tab, and Advanced is the depth after it (elements and biomes, the five-level run, tactics, every die, and the complete rulebook as the last slide). Arrow keys, swipe, or the numbered rail
- **Cards**: all 111 faces, browsable by deck; tap one for its name and what it does, because the face never says
...
| **[data/cards.json](data/cards.json)** | All 111 cards as data. |
...
70 of the 75 art slots carry credited Noun Project icons, from 29 creators, all CC BY 3.0.
...
│   └── views/              # 10 files: learn · cards · balance · about · play · play-plan · play-board · play-screens · inspect · logline
├── data/
│   ├── cards.json          # all 111 cards, the one source
│   └── art-manifest.json   # 75 art slots with creator + licence per slot
├── tools/                  # sim.py · lint_cards.py · dice_bridge.py · credits.py
├── tests/                  # 9 suites: browse · cards · content · dice · engine (BALANCE.md parity) · learn · logline · play · seam
